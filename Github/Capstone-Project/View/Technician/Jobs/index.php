     ...
     <main class="flex-1 min-w-0 transition-all duration-300 ease-in-out mt-14 px-6 py-10" :class="{'lg:ml-64': sidebarOpen}">
        <div>
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-500 mt-1">Manage your assigned service repairs.</p>
            </div>

            <!-- Stat Cards -->
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-5 mb-8">
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                    ...
                    <p class="text-xs text-gray-400 mt-1">All assigned jobs</p>
                </div>
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                    <div class="flex items-center justify-between mb-3">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ jobs.filter(j=>j.status==='Pending').length }}</p>
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ jobs.filter(j=>j.status==='Completed').length }}</p>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        <h2 class="text-base font-bold text-gray-800">All Jobs</h2>
                        <p class="text-sm text-gray-400">Your complete assignment list</p>
                    </div>
                    <span class="text-xs font-semibold bg-gray-100 text-gray-600 px-3 py-1 rounded-full">{{ jobs.length }} total</span>
                </div>

                <div v-if="loading" class="px-6 py-14 text-center text-gray-400">Loading jobs...</div>
                <div v-else-if="jobs.length === 0" class="px-6 py-14 text-center text-gray-400">
                ...
                <div v-else class="divide-y divide-gray-50">
                    <div v-for="job in jobs" :key="job.id" class="px-6 py-5 hover:bg-gray-50 transition-colors flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div class="flex items-center gap-4 min-w-0">
                            <div class="w-11 h-11 rounded-full bg-emerald-100 flex items-center justify-center text-sm font-bold text-emerald-700 flex-shrink-0">
                                {{ (job.customer_name || 'C').charAt(0) }}
                            </div>
                            <div class="min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <h4 class="font-semibold text-gray-800">{{ job.customer_name }}</h4>
                                    <span class="text-xs px-2.5 py-0.5 rounded-full font-semibold"
                                         :class="{'bg-yellow-100 text-yellow-700': job.status==='Pending', 'bg-blue-100 text-blue-700': job.status==='In Progress', 'bg-emerald-100 text-emerald-700': job.status==='Completed'}">
                                        {{ job.status }}
                                    </span>
                                </div>
                                <p class="text-sm text-gray-400 mt-0.5 truncate">{{ job.address }}</p>
                                <p class="text-xs text-gray-300 mt-0.5">Requested {{ job.created_at }}</p>
                            </div>
                        </div>
                        <button class="flex-shrink-0 inline-flex items-center gap-1.5 text-xs font-semibold bg-emerald-50 text-emerald-600 px-4 py-2 rounded-xl hover:bg-emerald-100 transition">
                            View Details
...
<script>
    ...
    createApp({
            ...
            return {
                ...
                menuGroups: [
                    ...
                    { title: 'Services', isOpen: true, items: [
                        { name: 'Job Requests', icon: 'jobs', link: '#', active: true, badge: true },
        ...
        methods: {
            ...
            async initData() {
                try {
                    await Promise.all([this.loadProfile(), this.loadJobs()]);
                } finally {
                    this.loading = false;
                }
            },
