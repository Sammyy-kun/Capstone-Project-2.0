const app = Vue.createApp({
    data() {
        return {
            sidebarOpen: true,
            user: {
                //Here dapat kukunin yung name from database kase dynamic
                name: 'Owner',
                image: '../../../Public/pictures/profile pic.jpg'
            },
            metrics: {
                users: 0,
                owners: 0,
                technicians: 0,
                revenue: 0
            },
            menuGroups: [
                {
                    title: 'Main',
                    isOpen: true,
                    items: [
                        { name: 'Dashboard', icon: 'dashboard', link: '../Dashboard/dashboard.php', active: true }
                    ]
                },
                {
                    title: 'Management',
                    isOpen: true,
                    items: [
                        { name: 'Business Apps', icon: 'categories', link: '../Business/application.php' },
                        { name: 'Business Partners', icon: 'partners', link: '../Business/partners.php' }
                    ]
                },
                {
                    title: 'Repair Services',
                    isOpen: true,
                    items: [
                        { name: 'Technicians', icon: 'technicians', link: '../Technicians/index.php' }
                    ]
                },
                {
                    title: 'Users',
                    isOpen: true,
                    items: [
                        { name: 'Accounts', icon: 'users', link: '../Accounts/accounts.php' }, // Was Users
                    ]
                },
                {
                    title: 'System',
                    isOpen: true,
                    items: [
                        { name: 'Schedule', icon: 'reports', link: '../Schedule/index.php' },
                        // { name: 'Reports', icon: 'reports', link: '#' },
                        // { name: 'Settings', icon: 'settings', link: '#' },
                        // { name: 'Logs', icon: 'logs', link: '#' }
                    ]
                }
            ],
            logoutLink: { name: 'Logout', icon: 'logout', link: '../../Auth/logout.php' }
        }
    },
    methods: {
        toggleSidebar() {
            this.sidebarOpen = !this.sidebarOpen;
        },
        toggleMenu(index) {
            this.menuGroups[index].isOpen = !this.menuGroups[index].isOpen;
        },
        getIcon(iconName) {
            const icons = {
                dashboard: 'M19 11H5M19 11C20.1046 11 21 11.8954 21 13V19C21 20.1046 20.1046 21 19 21H5C3.89543 21 3 20.1046 3 19V13C3 11.8954 3.89543 11 5 11M19 11V9C19 7.89543 18.1046 7 17 7M5 11V9C5 7.89543 5.89543 7 7 7M7 7V5C7 3.89543 7.89543 3 9 3H15C16.1046 3 17 3.89543 17 5V7M7 7H17',
                products: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4',
                categories: 'M4 5a1 1 0 011-1h4a1 1 0 011 1v7a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM14 5a1 1 0 011-1h4a1 1 0 011 1v7a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 16a1 1 0 011-1h4a1 1 0 011 1v3a1 1 0 01-1 1H5a1 1 0 01-1-1v-3zM14 16a1 1 0 011-1h4a1 1 0 011 1v3a1 1 0 01-1 1h-4a1 1 0 01-1-1v-3z',
                partners: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z',
                orders: 'M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z',
                payments: 'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z',
                repair: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065zM15 12a3 3 0 11-6 0 3 3 0 016 0z',
                technicians: 'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z',
                users: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z',
                admins: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
                reports: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
                settings: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z',
                logs: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
                logout: 'M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1'
            };
            return icons[iconName] || '';
        },
        handleImageError(event) {
            event.target.src = 'https://via.placeholder.com/300x300/f3f4f6/6b7280?text=No+Image';
        },
        async loadProfile() {
            try {
                const res = await fetch('../../../Controller/user-controller.php?action=get_profile');
                const data = await res.json();
                if (data.status === 'success') {
                    this.user.name = data.data.username || data.data.full_name;
                    this.user.image = data.data.profile_picture || this.user.image;
                }
            } catch (e) {
                console.error("Failed to load profile", e);
            }
        },
        async loadDashboardMetrics() {
            try {
                const res = await fetch('../../../Controller/user-controller.php?action=get_dashboard_metrics');
                const data = await res.json();
                if (data.status === 'success') {
                    this.metrics = data.data;
                }
            } catch (e) { console.error(e); }
        },
        toggleProfileMenu() {
            window.location.href = '../Profile/edit.php';
        },
        toggleNotifications() {
            alert("No new notifications");
        }
    },
    mounted() {
        this.loadProfile();
        // Check if we are on the main dashboard page
        if (window.location.pathname.includes('Dashboard/dashboard.php')) {
            this.loadDashboardMetrics();
        }
    }
})
app.mount('#app')